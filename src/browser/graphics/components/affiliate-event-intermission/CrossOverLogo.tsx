...
export const CrossOverLogo = ({ className }: Props) => {
  ...
  switch (currentDayLogo) {
    ...
    case "Yakuza":
      ...
      break;
    case "Xenogears":
      currentImage = Xenogears;
      break;
    case "Saga":
      currentImage = Saga;
      break;
    case "Persona":
      currentImage = Persona;
      break;
    default:
      currentImage = logo;
      break;
  ...
  return (
    <div
      className={`w-full h-full flex flex-row p-2 items-center justify-center gap-5 ${className}`}
    >
      <img src={Logo} className="h-full object-contain" alt="Logo" />
      <h2
        className={`drop-shadow text-5xl font-thin ${className}`}
        style={{
      ...
      <img
        src={currentImage}
        className="max-w-[800px] h-full object-contain drop-shadow-white drop-shadow-xs"
        alt="Logo"
      />
    </div>
  );
