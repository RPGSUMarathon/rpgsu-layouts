...
runDataActiveRun.on(
  "change",
  (
    newVal: RunDataActiveRun | undefined,
    oldVal: RunDataActiveRun | undefined,
  ) => {
    ...
    if (newVal && oldVal && newVal.id === oldVal.id) {
      nodecg.log.debug(
        "Run ID %s did not change, not updating layout",
        newVal.id,
      );
      return;
    }
    ...
    if (newVal) {
      ...
      if (newVal.customData && newVal.customData.layout) {
        ...
        nodecg.log.warn(
          "Run ID %s does not have custom data for layout",
          newVal.id,
        );
        return;
      }
